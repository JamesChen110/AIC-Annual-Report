...
namespace BrowserAutomation
{
    public class clsBrowserAutomation
    {
        ...
        //string strchromedriver= Environment.CurrentDirectory;
        string strchromedriver = @"C:\AIC Annual Report";
        /// <summary>
        ...
        public IWebDriver openOpera(bool p_clnCloseOpenedOperaDriver = true)
        {
            ...
            if (Environment.Is64BitOperatingSystem == true)
                strDriverLocation = strchromedriver + "\\operadriver_win64";
            else
                strDriverLocation = strchromedriver + "\\operadriver_win32";
        ...
        public IWebDriver openIE(bool p_clnCloseOpenedChromDriver = true, string p_strDriverLocation = "")
        {
            ...
            {
                if (Environment.Is64BitOperatingSystem == true)
                    strDriverLocation = strchromedriver + "\\IEdriver_win64";
                else
                    strDriverLocation = strchromedriver + "\\IEdriver_win32";
            }
    ...
    #region Get/Set Values
        public void SetText(IWebDriver p_IWebDriver, string p_strControlID, string p_strText,Boolean blNeedSendEnterKey=false)
        {
            try
            {
                //p_IWebDriver.FindElement(By.Id(p_strControlID)).Click();
                p_IWebDriver.FindElement(By.Id(p_strControlID)).Clear();
                p_IWebDriver.FindElement(By.Id(p_strControlID)).SendKeys(p_strText);
                if(blNeedSendEnterKey) p_IWebDriver.FindElement(By.Id(p_strControlID)).SendKeys(Keys.Enter);
        ...
        public void SetText_byIWebElement(IWebElement p_IWebElement, string p_strText, Boolean blNeedSendEnterKey = false,string strAfterSendKey="", Boolean blnNeedCheckInputText =false)
        {
            try
            {
                //p_IWebDriver.FindElement(By.Id(p_strControlID)).Click();
                //select element 也可以使用sendkey ，但是不能clear
                if (p_IWebElement == null)
                {
                    throw new Exception("没有在网站找到该字段");
                ...
                p_IWebElement.SendKeys(p_strText);


                if (blnNeedCheckInputText)
                {
                    if (GetText_byIWebElement(p_IWebElement) == p_strText)
                    {

                    }
                    else
                    {
                        throw (new Exception("显示值不等于输入的值"));
                ...
                if (blNeedSendEnterKey) p_IWebElement.SendKeys(Keys.Enter);

                if (strAfterSendKey == null || strAfterSendKey == "")
                { }
                else
                {
                    if (strAfterSendKey.ToLower() == "tab")
                    {
                        p_IWebElement.SendKeys(Keys.Tab);
                    }
                    else if(strAfterSendKey.ToLower() == "enter")
                    {
                        p_IWebElement.SendKeys(Keys.Enter);
        ...
        public void SetText_ByName(IWebDriver p_IWebDriver, string p_strControlName, string p_strText, Boolean blNeedSendEnterKey = false)
        {
            try
            {
                p_IWebDriver.FindElement(By.Name(p_strControlName)).Clear();
                p_IWebDriver.FindElement(By.Name(p_strControlName)).SendKeys(p_strText);
                p_IWebDriver.FindElement(By.Name(p_strControlName)).SendKeys(Keys.Enter);
        ...
        public string GetText(IWebDriver p_IWebDriver, string p_strControlID)
        {
            ...
            try
            {
                ...
                if (strValue == null || strValue == "")
                    strValue = iwebEle.GetAttribute("value");
        ...
        public string GetText_byIWebElement(IWebElement p_IWebElemen)
        {
            ...
            try
            {
                ...
                string strValue = p_IWebElemen.GetAttribute("Value");
                if (strValue == null || strValue == "" || strValue == "0")
                    strValue = p_IWebElemen.GetAttribute("value");

                if (strValue == null || strValue == "")
                    strValue = p_IWebElemen.Text;
        ...
        public string GetText_ByName(IWebDriver p_IWebDriver, string p_strControlName)
        {
            IWebElement iwebEle;
            try
            {
                iwebEle = p_IWebDriver.FindElement(By.Name(p_strControlName));
        ...
        public void SelectElement_SelectText(IWebElement p_IWebElement, string p_strValue, Boolean blNeedSendCtrl = false)
        {
            SelectElement selectControl = new SelectElement(p_IWebElement);


            if (! p_IWebElement.Selected)
            {

                selectControl.SelectByValue(p_strValue);
            }


        }

        public void checkboxElement_SelectText(IList<IWebElement>  p_litIwebEle, string p_strValue,string p_strSeachType="value")
        {


            try
            {
                foreach (IWebElement item in p_litIwebEle)
                {
                    if (p_strSeachType == null || p_strSeachType == "value")
                    {
                        if (item.GetAttribute("value").Contains(p_strValue))
                        {
                            if (!item.Selected)
                            {
                                item.Click();
                    ...
                    else if (p_strSeachType == "text")

                    {


                        if (item.Text.Contains(p_strValue))
                        {
                            if (!item.Selected)
                            {
                                item.Click();
        ...
        public void ClickElementByLoopFind(IList<IWebElement> p_litIwebEle, string p_strValue, string p_strSeachType = "text")
        {
            ...
            try
            {
                foreach (IWebElement item in p_litIwebEle)
                {
                    if (p_strSeachType == "value")
                    {
                        if (item.GetAttribute("value") == p_strValue)
                        {
                            item.Click();
                        }
        ...
        public void checkboxElement_SelectTextByText(IList<IWebElement> p_litIwebEle, string p_strValue)
        {
        ...
        /// <param name="strControlID"></param>
        /// <param name="strSelectText"></param>
        public void SelectItem_ByValue(IWebDriver p_IWebDriver, string strControlID, string strSelectValue)
        {
            SelectElement SelectElement = new SelectElement(p_IWebDriver.FindElement(By.Id(strControlID)));
           
            //Select the option
            if (SelectElement.SelectedOption.GetProperty("value") != strSelectValue)
                SelectElement.SelectByValue(strSelectValue);

            //Validation
            if (SelectElement.SelectedOption.Text != strSelectValue)
            {
                throw new Exception("Validation failed for selecting the text:" + strSelectValue + " for Control: " + strControlID);
            }
        }
